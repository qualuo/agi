...
After each task the agent runs a small reflection step: a cheap LLM call
that looks at the user prompt, the assistant's final answer, and a summary
of tool usage, and proposes 0–3 lessons to commit to long-term memory
tagged `lesson`. The lessons surface on the next related task via memory
search.

This is the simplest channel of medium-timescale learning that survives
across sessions without weight updates: explicit, inspectable notes the
agent reads at the start of related work.

Cost ceiling: one Haiku call per turn, max ~600 tokens. Falls open if the
API call fails — reflection is a nice-to-have, not blocking. Off by default
in `SessionConfig`; flip `enable_reflection=True` to opt in.
"""
...
def _parse_lessons(text: str) -> list[dict]:
    ...
    accepts strict JSON, JSON embedded in prose, or returns [] on garbage.
    Caps at 3 lessons, 300 chars each, 6 tags each.
    """
    if not text.strip():
        return []
    try:
