...
the user has mentioned. Each observation is timestamped with an outcome so
the agent can answer "have I seen this before?" and "what happened last
time?" without re-deriving from raw conversation history.

This is intentionally minimal — it's a structured log of interactions, not
a causal graph. ARCHITECTURE.md is honest about the gap: a proper grounded
world model is open research. What this module does buy:

- A coordinator can plan over real state ("the runtime has read /etc/foo,
  last result was failure; warn before retrying").
- The agent can avoid re-doing expensive work (fetched this URL last hour,
  use the cached observation instead).
- A skill-mining pass has typed signal beyond raw text traces.

Persisted as JSONL; the latest observation per (kind, id) is held in memory
for O(1) lookup and reloaded on construction.
"""
        ...
        with self.path.open() as f:
            for line in f:
                ...
                try:
                    ...
                    # Malformed line — skip rather than break the load.
                    continue
